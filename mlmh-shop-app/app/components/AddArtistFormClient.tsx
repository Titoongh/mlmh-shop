...
import type {
    ArtistFormData,
    AddArtistFormClientProps,
} from './AddArtistForm.types'

export default function AddArtistFormClient({
    id,
    mode = 'create',
    onSuccess,
    ...
    // Initialize picture based on initial data
    const [picture, setPicture] = useState<ContentFormData>(() => {
        if (mode === 'update' && initialArtist?.contents?.[0]) {
            return {
                type: 'IMAGE',
                url: initialArtist.contents[0].url || '',
                rank: 1,
                uploadType: 'url',
            }
        }
    ...
    const [formData, setFormData] = useState<ArtistFormData>(() => {
        if (mode === 'update' && initialArtist) {
            return {
                name: initialArtist.name,
                description: initialArtist.description || '',
                musicalGenres:
                    initialArtist.musicalGenres?.map(g => g.id) || [],
                hidden: initialArtist.hidden,
            }
        }
        return {
            name: '',
    ...
    return (
        <div className='w-full max-w-2xl'>
            <h2 className='mb-6 text-2xl font-bold'>
                {mode === 'create' ? 'Add New Artist' : 'Update Artist'}
            </h2>
            {error && (
